...
/*
** 对请求参数进行组装、编码、签名，返回已经组装好签名的参数字符串
** @params {Object}   请求参数
** @params {String}   商户应用私钥
** @params {String}   [signType] 签名类型 RSA || RSA2
** @returns {String}
*/ 
...
utl.signVerify = function(str,sign,publicKey,signType){
    ...
    if(signType == "RSA2"){
        ...
        verify = crypoto.createVerify("RSA-SHA1")
    }
...
utl.request = function (opts){
    return new Promise(function(resolve,reject){
        request(opts,function(err,response,body){
            ...
            let ret = {response:response,body:body};
            ret.ok = function(){
...
utl.alipay_verify = function(params){
    var preStr = "";
    var preArr = [];
    ...
    for(var i=0,len=keys.length;i<len;i++){
        ...
        preArr.push(keys[i]+"="+decodeURIComponent(params[keys[i]]));
    } 
    preStr = preArr.join("&");
    var result = crypto.createVerify("RSA-SHA256")
                        .update(preStr)
